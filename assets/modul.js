// cek login dulu, kalau belum login langsung redirect
...
document.addEventListener("DOMContentLoaded", () => {
  const courseData = [
    { title: "Introduction to C Programming", lessons: [{ title: "Pengenalan Bahasa C", xp: 50 }] },
    { title: "Operators", lessons: [{ title: "Mengenal Operator di C", xp: 75 }] },
    { title: "Selection", lessons: [{ title: "Struktur Seleksi (If-Else & Switch)", xp: 100 }] },
    { title: "Repetition", lessons: [{ title: "Struktur Perulangan (For, While, Do-While)", xp: 100 }] },
    { title: "String Manipulation", lessons: [{ title: "Manipulasi String", xp: 120 }] },
    { title: "Pointer and Array", lessons: [{ title: "Pointer dan Array", xp: 150 }] },
    { title: "Midterm Exam Simulation", lessons: [{ title: "Simulasi Ujian Tengah Semester", xp: 200 }] },
    { title: "Function", lessons: [{ title: "Membuat dan Menggunakan Fungsi", xp: 100 }] },
    { title: "Recursion", lessons: [{ title: "Fungsi Rekursif", xp: 125 }] },
    { title: "Structures", lessons: [{ title: "Bekerja dengan Struct", xp: 120 }] },
    { title: "File Processing", lessons: [{ title: "Membaca dan Menulis File", xp: 150 }] },
    { title: "Sorting 1", lessons: [{ title: "Algoritma Sorting Dasar", xp: 130 }] },
    { title: "Sorting 2", lessons: [{ title: "Algoritma Sorting Lanjutan", xp: 150 }] },
    { title: "Searching", lessons: [{ title: "Algoritma Pencarian", xp: 120 }] },
    { title: "Final Exam Simulation", lessons: [{ title: "Simulasi Ujian Akhir Semester", xp: 300 }] },
  ];

  const badgeData = [
    { title: "C Initiate", unlockIndex: 3,
    ...
    { title: "Midterm Survivor", unlockIndex: 6,
    ...
    { title: "Data Architect", unlockIndex: 10,
      icon: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>` },
    { title: "C Master", unlockIndex: 14,
      icon: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11.33V8l-5 4 5 4v-3.33"/><path d="M15 12.67V16l5-4-5-4v3.33"/><path d="M12 18V6"/></svg>` },
  ];
  ...
  // generate avatar dari inisial username kalau belum upload foto
  ...
  const renderAll = () => {
    if (moduleContainer) {
      ...
      courseData.forEach((module, moduleIndex) => {
        ...
        const card = `<details class="module-card" ${openAttribute}>
          <summary class="flex items-center justify-between p-4 cursor-pointer rounded-t-xl">
            <div class="flex items-center space-x-4">
              <span class="flex items-center justify-center w-8 h-8 rounded-full ${headerColorClasses} font-bold">${moduleIndex + 1}</span>
              <h2 class="text-lg font-semibold text-white">${module.title}</h2>
            </div>
            <svg class="chevron w-5 h-5 text-slate-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clip-rule="evenodd" /></svg>
          </summary>
          <div class="p-4 border-t border-slate-700/50 space-y-2">
            <div class="flex justify-between items-center">
              <span class="text-slate-300">${module.lessons[0].title}</span>
              <a href="materi.html?lesson=${lessonGlobalIndex}" class="lesson-item btn-3d-action" data-lesson-index="${lessonGlobalIndex}">${isCompleted ? "Lihat Lagi" : "Mulai"}</a>
            </div>
          </div>
        </details>`;
        moduleContainer.innerHTML += card;
        lessonGlobalIndex++;
      });
    ...
    const totalXp = courseData.reduce((sum, mod) => sum + mod.lessons[0].xp, 0);
