...
void printTable1(int row, int i){ // print table
	printf("----------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
	printf("||%-4s|%-36s|%-25s|%-15s|%-23s|%-12s|%-8s|%-15s|%-10s||\n", datList[0], datList[1], datList[2], datList[3], datList[4],datList[5],datList[6],datList[7],datList[8]);
	// for align left, the number is booking some place for the list
	// supaya data rapih
	printf("----------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
	for(int j=0;j<row && j<i;j++){ 
	// row == baris yang diinginkan 
	// i data maximum
	// 1 print jika masih termasuk data yang dicari
	// 2 syarat supaya kalau row yang diinginkan lebih dari maximum data tidak akan print data kosong
		printf("||%-4d|%-36s|%-25s|%-15d|%-23s|%-12d|%-8.1f|%-15s|%-10d||\n", j+1, list[j].restaurantName, list[j].city, list[j].costForTwo, list[j].currency, list[j].priceRange, list[j].rating, list[j].ratingText, list[j].votes);
	}
	printf("----------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
}
...
void exportData(int i, char head[1005]){
	...
	do{
		printf("Input file name : "); fflush(stdin);
		scanf("%[^\n]", flName); getchar();
		//fflush menghilangkan buffer input
		//tanpanya, do while dihitung salah, dan loop
...
//If u choose 3
void sortData(int i){
	char clm[100]; //column yang mau dicari
	char srch[10]; //variable jenis sort
	int l; //bakal disetting buat tentuin kolom
	int shw = 10; //setting tampilan tabel 10 pertama
	...
	do{
		...
		if(strcmp(srch, "asc")!=0 && strcmp(srch, "desc")!=0)printf("Invalid sort type. Reinput.\n");
	}while(strcmp(srch, "asc")!=0 && strcmp(srch, "desc")!=0); 
	if(l==1){ // restaurant name
		if(strcmp(srch,"asc")==0){
			for(int j=0;j<i;j++){
		...
		if(strcmp(srch,"asc")==0){
			...
			for(int j=0;j<i;j++){
				for(int k=0;k<i-j-1;k++){
					int z=0;
					while(list[k].ratingText[z]!='\0' && tolower(list[k].ratingText[z])==
					      tolower(list[k+1].ratingText[z])) z++;
					if(tolower(list[k].ratingText[z])<
					   tolower(list[k+1].ratingText[z])){
						temp=list[k];
						list[k]=list[k+1];
						list[k+1]=temp;
					}
...
void searchData(int i){
	char clm[100]; //input column yang ingin dicari
	char srchStr[105]; //inputan yang ingin dicari berupa string
	int srchInt; //inputan yang ingin dicari berupa integer
	float srchFloat; //bakal di setting buat nentuin pilihan column
	int l; //set nomor di tabel
	int k = 0;
	...
	// 3. Cost For Two (Range)
		...
		printf("Enter maximum cost: "); scanf("%d",&max);
	...
	// 4. Currency
		...
		if(srchDatCurrency(srchStr,i)){
			printf("Data found\n");
			printf("------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
			printf("||%-4s|%-30s|%-25s|%-15s|%-20s|%-12s|%-8s|%-15s|%-10s||\n",
				datList[0],datList[1],datList[2],datList[3],datList[4],
				datList[5],datList[6],datList[7],datList[8]);
			printf("------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
			...
			for(int j=0;j<i;j++){
				if(strcmp(srchStr,list[j].currency)==0){
					printf("||%-4d|%-30s|%-25s|%-15d|%-20s|%-12d|%-8.1f|%-15s|%-10d||\n",
						++k, list[j].restaurantName, list[j].city, list[j].costForTwo, list[j].currency, list[j].priceRange, list[j].rating, list[j].ratingText, list[j].votes);
				}
			}
			printf("------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
...
int main(){
	...
	fscanf(f, "%[^\n]\n", head);
	int i=0;
	// FIX: Loop pembacaan file yang lebih aman (cegah duplikasi baris terakhir)
